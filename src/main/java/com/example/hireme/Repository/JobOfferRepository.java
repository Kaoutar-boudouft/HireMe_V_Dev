...
public interface JobOfferRepository extends JpaRepository<JobOffer,Long> {
    ...
    @Query(value = "select * from jobs_offers jobs where active=b'1' and jobs.title like %?1% and category_id like ?3 and city_id in(select id from cities where country_id like ?2)", nativeQuery = true)
    List<JobOffer> findByTitleAndCategoryAndLocation(String title,String location,String category);

    @Query(value = "select * from jobs_offers jobs where active=b'1' and jobs.title like %?1% and category_id like ?3 and city_id in(select id from cities where country_id like ?2) limit ?4,?5", nativeQuery = true)
